...
void gfluid_data_copy(data_ptr in, data_ptr out) {
...
void gfluid_data_init_copy(data_ptr in, data_ptr out) {
  out->N = in->N;
  ffluid_init_data(out);
  gfluid_data_copy(in, out);
  gfluid_setup_grid(out->map, out->N);
  //ffluid_setup_grid(out);
}


void gfluid_data_fma(long_double_t op1, data_ptr op2, data_ptr op3, data_ptr out) {
  unsigned long N = op2->N;
  for (unsigned long j = 0; j < N; j++) {
    //out->Q[j] = cfmaq(op1, op2->Q[j], op3->Q[j]);
    //out->V[j] = cfmaq(op1, op2->V[j], op3->V[j]);
